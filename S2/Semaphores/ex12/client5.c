...
int main() {
	sem_t *sm_seller,*sm_buyer,*sinc; /* Controla a concorrencia dos filhos */;
	...
	if((sinc=sem_open("sinc_4",O_EXCL,0644,1))==SEM_FAILED) {
		perror("Erro no sm_open(sinc)\n");
		exit(1);
	}
	/* O primeiro client a chegar é o primeiro a ser atendido */
	if(sem_wait(sinc)==-1) { perror("Erro no sem_wait(sinc)\n"); exit(1);}
